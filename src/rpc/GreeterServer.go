...
func Say2Hello(context context.Context, in *helloworld.HelloRequest) (*helloworld.HelloReply, error) {
	time.Sleep(stuckDuration)
	return &helloworld.HelloReply{Message: "Hello " + in.Name + "! From " + GetIP()}, nil
}
...
func GreeterSerice() {
	...
	s := grpc.NewServer()
	helloworld.RegisterGreeterService(s, &helloworld.GreeterService{Say2Hello: Say2Hello})
